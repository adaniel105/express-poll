...
function addUserToSocketDataIfAuthenticated(socket, next) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            ...
            if (user) {
                try {
                    socket.data = __assign(__assign({}, socket.data), { user: user });
                }
                catch (err) { }
            }
            next();
...
io.on('connection', function (socket) {
    ...
    socket.on('askForStateUpdate', function () {
        console.log('client asked For State Update');
        socket.emit('updateState', poll);
    });
    socket.on('vote', function (optionId) {
        // If user has already voted, remove their vote.
        poll.options.forEach(function (option) {
            option.votes = option.votes.filter(function (user) { return user !== socket.data.user; });
        });
        // And then add their vote to the new option.
        var option = poll.options.find(function (o) { return o.id === optionId; });
        if (!option) {
            return;
        }
